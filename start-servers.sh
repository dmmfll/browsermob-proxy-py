...
cd $TOOLS && \
java -jar $JAR -role hub -log log.txt &
cd $TOOLS && \
java -Dwebdriver.chrome.driver=$(which chromedriver) -Dwebdriver.gecko.driver=$(which geckodriver) -jar $JAR -role node -hub http://$NETLOC:4444/grid/register/ -log log_.txt &
